...
if [ "$PROJECTPWD" = "$FRAMEWORKPWD" ]; then
	...
	mkdir -p ./objects
	cd ./objects
	gcc -Wall -Wextra -Werror -include framework.h -c -g ../source/framework/*.c ../source/types/**/*.c -iquote ../source/framework $(find ../source/types -type d -exec echo -iquote {} \; | tr '\n' ' ' | sed 's/.$//')
