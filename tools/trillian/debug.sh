...
if [ "$PROJECTPWD" = "$FRAMEWORKPWD" ]; then
	...
	mkdir -p ./objects
	cd ./objects
	gcc -Wall -Wextra -Werror -include ft_framework.h -DDEBUG -c -g ../source/base/processes/**/*.c ../source/base/types/**/*.c ../source/processes/**/*.c ../source/types/**/*.c -iquote ../includes $(find ../source/base/types -type d -exec echo -iquote {} \; | tr '\n' ' ' | sed 's/.$//') $(find ../source/base/processes -type d -exec echo -iquote {} \; | tr '\n' ' ' | sed 's/.$//') $(find ../source/processes -type d -exec echo -iquote {} \; | tr '\n' ' ' | sed 's/.$//') $(find ../source/types -type d -exec echo -iquote {} \; | tr '\n' ' ' | sed 's/.$//')
